...
// 옮긴 것. 문서가 명시한 대로 "첫 버전부터 임의 Slot을 많이 만들지 않는다" — page.header/actions/
// summary/toolbar/footer 등 문서가 나열한 다른 표준 Slot은 지금 어떤 스켈레톤도 실제로 쓰지 않으므로
// 추가하지 않는다. RESOURCE_LIST/LIST_DETAIL은 PreviewBlockResolver와 마찬가지로 동일하게 취급한다.
