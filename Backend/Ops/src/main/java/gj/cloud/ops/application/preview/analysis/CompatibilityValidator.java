...
// Slot Cardinality(SlotCardinalityValidator), Component Contract 준수(ComponentContracts) → ERROR,
// §6 "로그인" 규칙 4 "최소 한 개 Username-like Field와 Password Field 필요"의 축소판 → WARNING.
// ERROR는 09-registry-lifecycle.md §11의 PROJECT+VALIDATED 승격 게이트를 막고, 그 외는
// PreviewAnalysisResult.warnings에만 표시된다 — Rule이 Blueprint를 자동 수정하지 않고 Finding만
// 반환한다는 §5 원칙 그대로, 사용자가 보고 판단하게 한다.
public final class CompatibilityValidator {

    public static List<CompatibilityFinding> validate(PageDraft page, List<Block> blocks, List<Capability> capabilities) {
        List<CompatibilityFinding> findings = new ArrayList<>();
        for (String violation : SlotCardinalityValidator.validate(page.skeleton(), blocks)) {
            findings.add(new CompatibilityFinding(CompatibilitySeverity.ERROR, violation));
        }

        for (Block block : blocks) {
            ComponentContract contract = ComponentContracts.ALL.get(block.componentId());
            if (contract == null) {
                findings.add(new CompatibilityFinding(CompatibilitySeverity.ERROR,
                        "등록되지 않은 componentId: " + block.componentId()));
                continue;
            }
            if (!contract.acceptedSurfaces().contains(block.slot())) {
                findings.add(new CompatibilityFinding(CompatibilitySeverity.ERROR,
                        block.componentId() + "는 " + block.slot() + " Slot을 받지 않음"));
            }
            ...
            if (block.componentId().equals("login-form")) {
                block.capabilityIds().stream()
                        ...
                        .ifPresent(login -> findings.add(new CompatibilityFinding(CompatibilitySeverity.WARNING,
