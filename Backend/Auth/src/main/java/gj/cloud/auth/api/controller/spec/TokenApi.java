...
public interface TokenApi {
    ...
    @Operation(summary = "토큰 갱신", description = "쿠키의 refreshToken으로 accessToken + 새 refreshToken 재발급 (Rotation)")
    ...
    @Operation(summary = "서비스 토큰 발급", description = "client_id/client_secret으로 인증하는 서비스 간 내부 호출용 토큰 발급 (client credentials)")
    @PostMapping("/token/service")
    ApiResponse<TokenResponse> issueServiceToken(@Valid @RequestBody ServiceTokenRequest request);
