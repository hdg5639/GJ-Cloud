...
| **Auto Preview** | OpenAPI 문서 하나로 로그인·목록·상세·생성·삭제 화면을 규칙 기반으로 자동 생성(AI 미사용)해 실제 대상 API에 시험 호출해보고, 확정하면 기존 배포 파이프라인으로 VM에 그대로 배포 |
...
OpenAPI 문서 URL 입력
    │
    ▼  분석 (AI 미사용 — 결정론적 규칙 기반)
    │
    목록/상세/생성/수정/삭제/로그인 Capability 추론
    → 검색 파라미터·페이지네이션·인증 방식(Bearer/API Key)·목록 응답 구조 감지
    → 로그인/대시보드/목록/상세 화면 구성(스켈레톤) 생성
    │
    ▼  마법사에서 확인
    │
    실제 대상 API로 로그인 → 목록 → 상세 → 생성 → 삭제까지 라이브 프리뷰로 직접 호출해보고,
    자동 감지가 애매한 값(access token 위치 등)은 직접 지정
    │
    ▼  확정 시 배포
    │
    Vite + React 프로젝트를 생성해 기존 배포 파이프라인으로 VM에 그대로 배포
    (Git 저장소 없는 배포 대상으로 취급). 배포 시점의 구성은 배포 기록에 함께 저장돼
    나중에 재분석 없이 조회 가능
```

라이브 프리뷰(브라우저)와 실제 배포되는 코드는 물리적으로 다른 산출물이지만, 화면 조립 규칙·인증 헤더 구성·응답 파싱 로직을 의도적으로 동일하게 유지해 마법사에서 미리 본 화면과 실제 배포된 화면이 어긋나지 않게 한다.

현재는 고정된 6종 화면 패턴(로그인/목록/상세/생성-수정/삭제/대시보드)만 지원하며, Cookie 기반 인증과 커스텀 UI 컴포넌트는 다루지 않는다. 여러 사용자의 실사용 데이터를 기반으로 한 품질 점수·공유 템플릿 추천 같은 후속 기능은 아직 실사용량이 없어 설계만 해두고 보류한 상태다.
