...
저장소에 기존 Dockerfile이 있고 숫자 `EXPOSE`가 선언돼 있으면 그 포트를 명시적 실행 계약으로 취급한다. Node 3000, Spring 8080 같은 프레임워크 기본값이나 분석 전 화면의 초기값이 이를 덮어쓰지 않으며, 생성된 Compose 포트·Caddy upstream·헬스체크가 모두 같은 포트를 사용한다. `EXPOSE`가 없을 때만 애플리케이션 설정과 런타임 기본값 순서로 추론한다.
...
배포 실행 중에는 이미지 빌드·태깅, DB 기반 SSE 실시간 로그(재접속 시 이벤트 재생), 헬스체크 실패 시 자동 롤백이 이뤄진다. 로그는 Git mirror·fetch·worktree, 파일 전송, Compose 검증, 서비스별 이미지 빌드, Compose up, 헬스체크 재시도, 라우트, 롤백을 개별 단계로 나눠 대상·종료 코드·소요 시간·제한된 stdout/stderr를 보여준다. 명령 원문은 저장하지 않고 Authorization, GitHub 토큰, 일반적인 secret 할당값은 저장·표시 경계에서 마스킹한다. Ops는 Open EntityManager in View를 끄고 이벤트 조회 트랜잭션과 장기 SSE 연결의 수명을 분리한다. 스트림에는 15초 heartbeat와 5분 상한을 적용하며, 정상 재연결 시 마지막 sequence 이후 이벤트만 이어받는다. 실패한 배포는 같은 스펙으로 재시도하거나 값을 수정 후 재배포할 수 있고, 과거 성공한 배포로 수동 롤백도 가능하다(재빌드 없이 해당 시점 이미지로 컨테이너만 재기동).
