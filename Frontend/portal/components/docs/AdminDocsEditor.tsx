    ...
    <form onSubmit={handleSubmit} onKeyDown={handleShortcut} className="mx-auto max-w-[1560px] text-[#1d2720]">
      ...
      <header className="sticky top-0 z-30 -mx-4 -mt-5 mb-5 flex flex-col gap-3 border-b border-[#dce4de] bg-white/95 px-4 py-3 backdrop-blur-xl sm:-mx-7 sm:-mt-7 sm:px-7 lg:-mx-[38px] lg:-mt-[34px] lg:mb-6 lg:flex-row lg:items-center lg:justify-between lg:px-[38px]">
        <div className="flex min-w-0 w-full items-center gap-3 lg:w-auto">
          <Link href="/docs" className="grid h-9 w-9 shrink-0 place-items-center rounded-[9px] border border-[#d9e1db] text-sm text-[#647168] hover:bg-[#f2f6f3]">←</Link>
          <div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-sm font-extrabold">{draft.title || "제목 없는 문서"}</h1><span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-extrabold ${article?.status === "PUBLISHED" ? "bg-[#e6f6ea] text-[#28753c]" : "bg-[#eff2ef] text-[#727d75]"}`}>{article?.status === "PUBLISHED" ? "발행됨" : "초안"}</span>{dirty && <span className="h-1.5 w-1.5 rounded-full bg-[#e5a83a]" title="저장하지 않은 변경" />}</div><p className="mt-0.5 text-[10px] text-[#909b93]">{notice ?? (dirty ? "저장하지 않은 변경 사항이 있습니다" : "모든 변경 사항이 저장됨")}</p></div>
        </div>
        <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center lg:shrink-0"><div className="col-span-2 grid grid-cols-3 rounded-[9px] border border-[#dce3dd] bg-[#f6f8f6] p-1 sm:flex">{(["WRITE", "SPLIT", "PREVIEW"] as EditorMode[]).map((item) => <button key={item} type="button" onClick={() => setMode(item)} className={`rounded-[7px] px-3 py-2 text-[10px] font-extrabold sm:py-1.5 ${mode === item ? "bg-white text-[#24432d] shadow-sm" : "text-[#7f8982]"}`}>{item === "WRITE" ? "작성" : item === "SPLIT" ? "분할" : "미리보기"}</button>)}</div><Button type="submit" disabled={saving || !dirty} className="w-full border-[#d8e0da] bg-white text-[#435048] hover:bg-[#f1f6f2] sm:w-auto">{saving ? "저장 중..." : "저장"}</Button><Button type="button" variant={article?.status === "PUBLISHED" ? "secondary" : "primary"} disabled={saving} onClick={togglePublish} className="w-full sm:w-auto">{article?.status === "PUBLISHED" ? "발행 취소" : "저장 후 발행"}</Button></div>
      </header>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        <main className="min-w-0 overflow-hidden rounded-[18px] border border-[#dce4de] bg-white shadow-sm">
          <div className="border-b border-[#e4eae5] p-4 sm:p-8">
            <Input value={draft.title} onChange={(event) => update("title", event.target.value)} placeholder="문서 제목" maxLength={180} className="h-auto min-h-0 rounded-none border-0 bg-transparent px-0 text-2xl font-black tracking-[-.035em] text-[#18221b] shadow-none focus:ring-0 placeholder:text-[#b8c0ba] sm:text-3xl" />
            <Textarea value={draft.summary} onChange={(event) => update("summary", event.target.value)} placeholder="목록과 문서 상단에 표시할 한두 문장의 요약을 적어 주세요." maxLength={400} rows={2} className="mt-3 min-h-0 resize-none rounded-none border-0 bg-transparent px-0 py-0 text-sm leading-6 text-[#6f7b73] focus:ring-0" />
          </div>

          <div className="flex flex-wrap items-center gap-1 border-b border-[#e4eae5] bg-[#fbfcfb] px-4 py-2">
            {TOOLBAR.map((tool) => <button key={tool.title} type="button" title={tool.title} onClick={() => insertMarkdown(tool.before, tool.after)} className="grid min-h-8 min-w-8 place-items-center rounded-[7px] px-2 text-[11px] font-extrabold text-[#657168] hover:bg-[#ebf1ec] hover:text-[#2f7140]">{tool.label}</button>)}
            <span className="mx-1 h-5 w-px bg-[#dce3dd]" />
            <button type="button" onClick={() => chooseImage("CONTENT")} disabled={uploading} className="inline-flex min-h-8 items-center gap-1.5 rounded-[7px] px-2.5 text-[11px] font-extrabold text-[#657168] hover:bg-[#ebf1ec] hover:text-[#2f7140]">{uploading ? <Spinner className="h-3 w-3" /> : "▧"} 이미지</button>
            <span className="ml-auto hidden text-[10px] text-[#a0aaa3] sm:block">Markdown · ⌘S 저장</span>
          </div>

          <div className={`grid min-h-[520px] sm:min-h-[680px] ${mode === "SPLIT" ? "lg:grid-cols-2" : "grid-cols-1"}`}>
            {mode !== "PREVIEW" && <div className={`min-w-0 ${mode === "SPLIT" ? "lg:border-r lg:border-[#e4eae5]" : ""}`}><textarea ref={textareaRef} value={draft.content} onChange={(event) => update("content", event.target.value)} spellCheck={false} className="h-full min-h-[520px] w-full resize-none bg-[#fcfdfc] p-4 font-mono text-[13px] leading-7 text-[#344139] outline-none placeholder:text-[#aab4ad] sm:min-h-[680px] sm:p-8" placeholder="Markdown으로 사용 설명서를 작성하세요." /></div>}
            {mode !== "WRITE" && <div className={`min-w-0 overflow-y-auto bg-white p-4 sm:p-8 lg:max-h-[820px] ${mode === "SPLIT" ? "hidden lg:block" : ""}`}><div className="mb-7 border-b border-[#e5ebe6] pb-6"><span className="text-[10px] font-extrabold uppercase tracking-[.12em] text-[#5f9a6c]">{draft.category || "카테고리"}</span><h1 className="mt-3 break-words text-2xl font-black tracking-[-.04em] sm:text-3xl">{draft.title || "문서 제목"}</h1><p className="mt-3 break-words text-sm leading-6 text-[#77837b]">{draft.summary || "문서 요약이 여기에 표시됩니다."}</p></div><MarkdownRenderer content={draft.content} resolveImageUrl={toAdminDocsImageUrl} className="[--foreground:#1d2720] [--muted:#66746b] [--line:#e1e8e3] [--brand:#68c77d] [--brand-strong:#318446]" /></div>}
          </div>
        </main>

        <aside className="grid gap-4 xl:sticky xl:top-20">
          <section className="rounded-[16px] border border-[#dce4de] bg-white p-4 shadow-sm"><h2 className="text-xs font-extrabold text-[#2c3730]">문서 설정</h2><div className="mt-4"><Field label="카테고리" htmlFor="docs-category"><Input id="docs-category" value={draft.category} onChange={(event) => update("category", event.target.value)} placeholder="예: 인스턴스, 배포, 협업" className="border-[#d9e1db] bg-[#fbfdfb] text-[#344038]" /></Field><Field label="문서 주소" htmlFor="docs-slug"><div className="flex items-center rounded-[9px] border border-[#d9e1db] bg-[#fbfdfb] px-3"><span className="text-xs text-[#9aa49d]">/docs/</span><input id="docs-slug" value={draft.slug ?? ""} onChange={(event) => update("slug", event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} placeholder="instance-create" className="min-w-0 flex-1 bg-transparent py-3 text-xs text-[#344038] outline-none" /></div><span className="text-[10px] font-normal text-[#929d96]">비워두면 제목을 기준으로 자동 생성됩니다.</span></Field><Field label="정렬 순서" htmlFor="docs-sort"><Input id="docs-sort" type="number" value={draft.sortOrder} onChange={(event) => update("sortOrder", Number(event.target.value))} className="border-[#d9e1db] bg-[#fbfdfb] text-[#344038]" /></Field><label className="flex items-start gap-3 rounded-[11px] border border-[#e0e6e1] bg-[#fafcfa] p-3"><input type="checkbox" checked={draft.featured} onChange={(event) => update("featured", event.target.checked)} className="mt-0.5 h-4 w-4 accent-[#64be78]" /><span><strong className="block text-xs">추천 가이드로 표시</strong><span className="mt-1 block text-[10px] leading-4 text-[#849087]">Docs 첫 화면 상단에 크게 노출합니다.</span></span></label></div></section>

          <section className="rounded-[16px] border border-[#dce4de] bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xs font-extrabold">커버 이미지</h2>{draft.coverImageUrl && <button type="button" onClick={() => update("coverImageUrl", null)} className="text-[10px] font-bold text-danger">제거</button>}</div>{draft.coverImageUrl ? <img src={toAdminDocsImageUrl(draft.coverImageUrl)} alt="" className="mt-3 aspect-[16/9] w-full rounded-[11px] border border-[#e0e6e1] object-cover" /> : <button type="button" onClick={() => chooseImage("COVER")} className="mt-3 grid aspect-[16/9] w-full place-items-center rounded-[11px] border border-dashed border-[#cad6cc] bg-[#f8fbf8] text-center"><span><span className="block text-xl text-[#7caa86]">▧</span><span className="mt-1 block text-[10px] font-bold text-[#7f8c83]">이미지 업로드</span></span></button>}{draft.coverImageUrl && <button type="button" onClick={() => chooseImage("COVER")} className="mt-2 w-full text-[10px] font-bold text-[#64816b] hover:text-[#347b44]">다른 이미지로 교체</button>}</section>

          <section className="rounded-[16px] border border-[#dce4de] bg-white p-4 shadow-sm"><h2 className="text-xs font-extrabold">태그</h2><div className="mt-3 flex gap-2"><Input value={tagInput} onChange={(event) => setTagInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTag(); } }} placeholder="태그 입력" className="min-h-9 border-[#d9e1db] bg-[#fbfdfb] text-xs text-[#344038]" /><button type="button" onClick={addTag} className="shrink-0 rounded-[9px] border border-[#d9e1db] px-3 text-xs font-bold text-[#56635a]">추가</button></div><div className="mt-3 flex flex-wrap gap-1.5">{draft.tags.map((tag) => <button key={tag} type="button" onClick={() => update("tags", draft.tags.filter((item) => item !== tag))} className="rounded-full bg-[#edf5ef] px-2.5 py-1.5 text-[10px] font-bold text-[#3e7449]">#{tag} ×</button>)}</div><p className="mt-2 text-[10px] text-[#929c95]">최대 12개 · 검색 키워드로도 사용됩니다.</p></section>
        </aside>
      </div>
    </form>
  );
