      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        <main className="min-w-0 overflow-hidden rounded-[18px] border border-[#dce4de] bg-white shadow-sm">
          ...
          <div className={`grid min-h-[520px] sm:min-h-[680px] ${mode === "SPLIT" ? "lg:grid-cols-2" : "grid-cols-1"}`}>
            ...
            {mode !== "WRITE" && <div className={`min-w-0 overflow-y-auto bg-white p-4 sm:p-8 lg:max-h-[820px] ${mode === "SPLIT" ? "hidden lg:block" : ""}`}><div className="mb-7 border-b border-[#e5ebe6] pb-6"><span className="text-[10px] font-extrabold uppercase tracking-[.12em] text-[#5f9a6c]">{draft.category || "카테고리"}</span><h1 className="mt-3 break-words text-2xl font-black tracking-[-.04em] sm:text-3xl">{draft.title || "문서 제목"}</h1><p className="mt-3 break-words text-sm leading-6 text-muted">{draft.summary || "문서 요약이 여기에 표시됩니다."}</p></div><MarkdownRenderer content={draft.content} resolveImageUrl={toAdminDocsImageUrl} className="[--foreground:#1d2720] [--muted:#56645b] [--line:#e1e8e3] [--brand:#68c77d] [--brand-strong:#318446]" /></div>}
          </div>
        </main>
