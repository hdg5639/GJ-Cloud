...
설정의 회원 탈퇴는 공통 모달에서 현재 비밀번호를 한 번 더 입력받는다. Auth 서버 검증이 성공한 경우에만 계정을 삭제하고 로그인 세션을 종료하며, 비밀번호가 틀리면 모달을 유지한 채 오류를 표시한다. 로그인과 탈퇴 확인이 429로 제한되면 `Retry-After`를 기준으로 남은 시간을 `분:초` 카운트다운으로 표시하고 버튼을 잠근다.
