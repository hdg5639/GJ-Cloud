...
export default function InstanceDetailPage() {
  ...
  useEffect(() => {
    ...
    api.vm.get(accessToken, id).then(setVm);
    api.vm.getSshAccess(accessToken, id).then((data) => setSshEmails(data.emails ?? [])).catch(() => {});
  ...
  const handleVmEvent = useCallback(
    (event: VmStatusEvent) => {
      if (event.vmId === id) {
        setVm((prev) =>
          prev
            ? { ...prev, status: event.status as VmResponse["status"], internalIp: event.internalIp ?? prev.internalIp }
            : prev
        );
      }
    },
  ...
  async function handlePower(action: "START" | "STOP" | "REBOOT") {
    if (!accessToken) return;
    try {
      await api.vm.power(accessToken, id, action);
    } catch (err) {
  ...
  async function handleAddPort(e: React.FormEvent) {
    ...
    try {
      ...
      setPortForm({ port: "", protocol: "HTTP", visibility: "PUBLIC", initialEmails: "" });
    } catch (err) {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => handlePower(vm.status === "RUNNING" ? "STOP" : "START")}
            disabled={!["RUNNING", "STOPPED"].includes(vm.status)}
            className="text-sm px-3.5 h-8 border border-gray-300 rounded-md disabled:opacity-40"
          >
            {vm.status === "RUNNING" ? "정지" : "시작"}
          </button>
          <button
      ...
      {vm.status === "FAILED" && vm.errorMessage && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-5">
          <p className="text-sm font-medium text-red-800">생성 실패</p>
      ...
      <div className="bg-gray-50 rounded-lg p-5 mb-4">
        ...
        >
          ...
          <span className={`text-gray-400 transition-transform duration-200 ${accordionOpen ? "rotate-180" : ""}`}>
            ▾
          </span>
        </button>
      ...
      <div className="bg-gray-50 rounded-lg p-5">
        ...
        {ports.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {ports.map((port) => (
              <div key={port.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-md px-3 py-2">
                <div>
                  <span className="text-sm font-medium text-gray-900">{port.port}</span>
                  <span className="text-xs text-gray-500 ml-2">
                    {port.protocol} · {port.visibility === "PUBLIC" ? "공개" : "비공개"}
                  </span>
                  <p className="font-mono text-[11px] text-gray-400">{port.fullDomain}</p>
                </div>
                <button
          ...
          <div className="bg-white rounded-xl p-6 w-[380px]">
            ...
            <form onSubmit={handleAddPort} className="flex flex-col gap-3">
              ...
              {portForm.visibility === "PRIVATE" && (
                <div>
                  <label className="text-xs text-gray-500 block mb-1">허용 이메일 (쉼표로 구분)</label>
                  <input
                    type="text"
              ...
              <div className="flex gap-2 mt-1">
                ...
                <button
                  type="submit"
                  disabled={portLoading || !portForm.port}
