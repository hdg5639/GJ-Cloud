  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[#7d8b81]">Support inbox</p>
          <h1 className="mt-2 text-3xl font-black tracking-[-.035em] text-[#162019]">사용자 문의</h1>
          <p className="mt-2 text-sm text-[#718078]">사용자가 남긴 문의를 확인하고 답변과 처리 상태를 관리합니다.</p>
        </div>
        <div className="rounded-[14px] border border-[#dce5de] bg-white px-4 py-3 text-right shadow-sm"><span className="block text-[10px] font-bold text-[#87938b]">현재 필터</span><strong className="mt-1 block text-xl text-[#1c2820]">{totalElements.toLocaleString("ko-KR")}건</strong></div>
      </header>
      ...
      <section className="mt-7 overflow-hidden rounded-[18px] border border-[#dce5de] bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-[#e4ebe6] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="hidden min-w-0 flex-wrap gap-2 sm:flex">
            {(["OPEN", "ANSWERED", "CLOSED", "ALL"] as StatusFilter[]).map((item) => (
              <button key={item} type="button" onClick={() => changeStatus(item)} className={`rounded-full border px-3.5 py-2 text-xs font-extrabold transition ${status === item ? "border-[#73c687] bg-[#e9f8ed] text-[#26733a]" : "border-[#dce4de] bg-[#fbfdfb] text-[#6f7c73] hover:bg-[#f2f7f3]"}`}>{item === "ALL" ? "전체" : STATUS_LABEL[item]}</button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="grid min-h-72 place-items-center px-5 text-center"><div><span className="text-3xl text-[#9eaaa1]">?</span><h2 className="mt-3 font-extrabold text-[#27322b]">표시할 문의가 없습니다</h2><p className="mt-1 text-sm text-[#829087]">다른 처리 상태를 선택해 보세요.</p></div></div>
        ) : (
                ...
                <article key={inquiry.id} className="grid min-w-0 gap-5 p-4 sm:p-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,440px)]">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-[#77847b]">{CATEGORY_LABEL[inquiry.category]}</span>
                      <time className="text-[10px] text-[#9aa49d]">{new Date(inquiry.createdAt).toLocaleString("ko-KR")}</time>
                    </div>
                    <h2 className="mt-3 break-words text-lg font-black text-[#1b2720]">{inquiry.title}</h2>
                    <div className="mt-2 flex min-w-0 flex-wrap gap-x-4 gap-y-1 text-[11px] text-[#87938b]"><span className="break-all">{inquiry.requesterEmail}</span><span className="break-all">사용자 ID: {inquiry.userId}</span></div>
                    ...
                    {inquiry.respondedAt && <p className="mt-3 text-[10px] text-[#929d96]">마지막 답변: {new Date(inquiry.respondedAt).toLocaleString("ko-KR")} · {inquiry.respondedBy}</p>}
                  </div>

                  <div className="min-w-0 rounded-[14px] border border-[#dfe6e1] bg-[#fbfdfb] p-4">
                    <label htmlFor={`response-${inquiry.id}`} className="flex items-center justify-between gap-3 text-xs font-extrabold text-[#455148]"><span>관리자 답변</span><span className="font-normal text-[#98a29b]">{(responses[inquiry.id] ?? "").length}/4000</span></label>
                    <Textarea id={`response-${inquiry.id}`} maxLength={4000} rows={7} value={responses[inquiry.id] ?? ""} onChange={(event) => setResponses((current) => ({ ...current, [inquiry.id]: event.target.value }))} placeholder="사용자에게 표시할 답변을 입력하세요." className="mt-2 min-h-36 resize-y border-[#d7e0d9] bg-white text-[#2d3931] placeholder:text-[#a0aaa3]" />
