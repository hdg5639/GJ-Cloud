  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[#7d8b81]">Content workspace</p>
          <h1 className="mt-2 text-3xl font-black tracking-[-.035em] text-[#162019]">사용 설명서 관리</h1>
          <p className="mt-2 text-sm text-[#718078]">사용자 포털에 노출할 기능별·페이지별 가이드를 작성하고 발행합니다.</p>
        </div>
      ...
      <section className="mt-7 grid gap-3 sm:grid-cols-4">
        {[
          ...
          <div key={String(label)} className="rounded-[16px] border border-[#dce5de] bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between"><span className="text-xs font-bold text-[#7b8880]">{label}</span><span className="text-[#77a482]">{icon}</span></div>
      ...
      <section className="mt-6 overflow-hidden rounded-[18px] border border-[#dce5de] bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-[#e4ebe6] p-4 sm:flex-row">
          <div className="relative min-w-0 flex-1"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#9aa69e]">⌕</span><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="제목, 카테고리, 태그 검색" className="border-[#d7e0d9] bg-[#fbfdfb] pl-9 text-[#253029]" /></div>
        ...
        {filtered.length > 0 ? (
            ...
            {filtered.map((article) => (
              <article key={article.id} className="group grid gap-4 p-4 transition hover:bg-[#f9fcfa] sm:grid-cols-[108px_minmax(0,1fr)_auto] sm:items-center">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 line-clamp-1 text-xs text-[#7a8780]">{article.summary}</p>
                  <div className="mt-2 flex flex-wrap gap-3 text-[10px] text-[#9aa49e]"><span>{new Date(article.updatedAt).toLocaleString("ko-KR")} 수정</span><span>조회 {article.viewCount.toLocaleString("ko-KR")}</span><span>/{article.slug}</span></div>
                </div>
          ...
          <div className="grid min-h-72 place-items-center text-center"><div><span className="text-3xl text-[#9eaaa1]">▤</span><h2 className="mt-3 font-extrabold text-[#27322b]">표시할 문서가 없습니다</h2><p className="mt-1 text-sm text-[#829087]">필터를 바꾸거나 첫 문서를 작성해 보세요.</p></div></div>
        )}
      </section>
      ...
      <Modal open={Boolean(deleteTarget)} onClose={() => !deleting && setDeleteTarget(null)}>
        <section className="mx-auto w-full max-w-md overflow-hidden rounded-[18px] border border-[#eadada] bg-white text-[#1e2822] shadow-2xl">
          <div className="p-6"><div className="grid h-11 w-11 place-items-center rounded-full bg-[#fff0f0] text-xl text-danger">!</div><h2 className="mt-5 text-xl font-black">문서를 삭제할까요?</h2><p className="mt-2 text-sm leading-6 text-[#758078]"><strong className="text-[#263129]">{deleteTarget?.title}</strong> 문서가 사용자 포털과 관리자 목록에서 완전히 삭제됩니다.</p></div>
